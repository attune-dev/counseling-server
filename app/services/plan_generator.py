...
4. 반드시 한국어로만 작성하세요
5. 질문은 나열이 아니라 깔때기 구조로 설계하세요. 앞 질문의 답변이 다음 질문으로 자연스럽게 이어지도록:
   - Step 1 (공감 형성): 2~5개 — 흐름: 표면 감정 탐색 → 감정 강도/빈도 → 감정의 맥락(언제, 어떤 상황) → 가장 힘든 부분 특정
   - Step 2 (문제 탐색): 3~7개 — 흐름: 가장 힘들었던 구체적 사건 특정 → 그 순간의 자동적 사고 포착 → 그 생각의 근거 탐색 → 비슷한 패턴 반복 여부 → 일상 영향 범위
   - Step 3 (사고 전환): 3~8개 — 흐름: 자동적 사고의 확신 정도(0~100%) → 반대 증거 탐색 → 타인 관점("친구라면?") → 대안적 사고 도출 → 새로운 관점의 확신 정도
   - Step 4 (행동 계획): 2~5개 — 흐름: 시도하고 싶은 것 탐색(열린 질문) → 구체적 실행 방법 → 예상 장애물 → 장애물 대처 방안. 반드시 열린 질문만.
   - Step 5 (마무리): 1~3개 — 흐름: 오늘 가장 인상 깊었던 통찰 → 실천 계획 확인
